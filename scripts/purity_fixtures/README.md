...
| Two temporary files generated by `--self-test` | 8, a decision-record identifier and a versioned development-branch identifier |
...
The two rule 8 samples are assembled only in the self-test's temporary scan
tree. Tracked source therefore does not need to carry the forbidden identifiers
in order to prove that the gate detects them.

...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size)
{
    ...
    double gamma = std::clamp(std::abs(buf[2]), 1e-3, 1e3);
    double a_m = std::clamp(buf[3], -0.95, 0.95);
    double sign_b = (buf[4] >= 0.0) ? 1.0 : -1.0;
    double dt = std::clamp(std::abs(buf[5]), 1e-3, 1.0);
    double theta_x0 = std::clamp(buf[6], -1.0, 1.0);
    double theta_r0 = std::clamp(buf[7], -1.0, 1.0);

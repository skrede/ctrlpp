...
| `fuzz_trapezoidal` | `valley_equal_boundary_long_realization.bin` | the counterexample's equal-boundary neighbour, which IS reachable: the retired parametrization accepted it and realized a duration 3.3e6 units in the last place LONG, where the shape-boundary parametrization lands within a fraction of one. The two seeds together are why an acceptance verdict alone was never evidence -- the retired form got the verdict wrong in one direction here and in the other on its neighbour, reporting success both times | `b03a1a2` | 7 little-endian binary64: `q0, q1, v_max, a_max, v0, v1, stretch`; minimum 56 bytes |

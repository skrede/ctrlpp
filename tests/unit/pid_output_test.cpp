...
TEST_CASE("saturated() returns true when output is clamped", "[pid][siso][saturated]")
{
    ...
    REQUIRE(pid.compute(vec1(0.1), vec1(0.0), Ts).has_value());
    ...
    "[pid][siso][setpoint-weighting]")
{
    ...
    // ep = 0*sp - meas = -0.3
    // P = 2.0 * (-0.3) = -0.6
    auto u = ctrlpp::test::commanded(pid.compute(vec1(1.0), vec1(0.3), Ts));
    ...
    "[pid][siso][setpoint-weighting]")
{
    ...
    REQUIRE(pid.compute(vec1(2.0), vec1(0.0), Ts).has_value());
    ...
    "[pid][siso][setpoint-weighting][derivative]")
{
    ...
    auto u2 = ctrlpp::test::commanded(pid.compute(vec1(3.0), vec1(0.0), Ts));

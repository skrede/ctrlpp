...
// bounds the accumulated rounding across the discretisation formula's matrix products/inverses.
...
Scalar discretise_tolerance(Scalar reference_magnitude)
{
    return static_cast<Scalar>(NX) * std::numeric_limits<Scalar>::epsilon() * (Scalar{1} + reference_magnitude);
}
}

TEST_CASE("tustin discretise maps continuous poles through the bilinear transform")
{
    // Upper-triangular A: eigenvalues are the diagonal entries s1 = -2, s2 = -5, and the
    // discretised Ad from a triangular A stays triangular, so its diagonal entries are the
    ...
    auto dsys = ctrlpp::discretise(ctrlpp::tustin{}, sys, dt);

    const double z1 = (1.0 + s1 * dt / 2.0) / (1.0 - s1 * dt / 2.0);
    const double z2 = (1.0 + s2 * dt / 2.0) / (1.0 - s2 * dt / 2.0);

    CHECK_THAT(dsys.A(0, 0), Catch::Matchers::WithinAbs(z1, discretise_tolerance<double, 2>(std::abs(z1))));
    CHECK_THAT(dsys.A(1, 1), Catch::Matchers::WithinAbs(z2, discretise_tolerance<double, 2>(std::abs(z2))));
    CHECK_THAT(dsys.A(1, 0), Catch::Matchers::WithinAbs(0.0, discretise_tolerance<double, 2>(0.0)));
}

TEST_CASE("forward euler discretise maps continuous poles through the first-order expansion")
{
    ctrlpp::continuous_state_space<double, 2, 1, 1> sys{};
    sys.A(0, 0) = -2.0;
    sys.A(0, 1) = 1.0;
    ...
    auto dsys = ctrlpp::discretise(ctrlpp::forward_euler{}, sys, dt);
    ...
    CHECK_THAT(dsys.A(0, 0), Catch::Matchers::WithinAbs(z1, discretise_tolerance<double, 2>(std::abs(z1))));
    CHECK_THAT(dsys.A(1, 1), Catch::Matchers::WithinAbs(z2, discretise_tolerance<double, 2>(std::abs(z2))));
    CHECK_THAT(dsys.A(0, 1), Catch::Matchers::WithinAbs(dt, discretise_tolerance<double, 2>(dt)));
}

TEST_CASE("backward euler discretise maps continuous poles through the resolvent")
{
    ctrlpp::continuous_state_space<double, 2, 1, 1> sys{};
    sys.A(0, 0) = -2.0;
    sys.A(0, 1) = 1.0;
    ...
    auto dsys = ctrlpp::discretise(ctrlpp::backward_euler{}, sys, dt);
...
TEST_CASE("tustin discretise matches the closed-form biproper scalar system")
{
    ...
    CHECK_THAT(dsys.A(0, 0), Catch::Matchers::WithinAbs(Ad, discretise_tolerance<double, 1>(std::abs(Ad))));
    CHECK_THAT(dsys.B(0, 0), Catch::Matchers::WithinAbs(Bd, discretise_tolerance<double, 1>(std::abs(Bd))));
    CHECK_THAT(dsys.C(0, 0), Catch::Matchers::WithinAbs(Cd, discretise_tolerance<double, 1>(std::abs(Cd))));
    CHECK_THAT(dsys.D(0, 0), Catch::Matchers::WithinAbs(Dd, discretise_tolerance<double, 1>(std::abs(Dd))));
}

TEST_CASE("forward euler discretise matches the closed-form scalar system")
{
    ctrlpp::continuous_state_space<double, 1, 1, 1> sys{};
    const double s = -3.0;
    const double b = 2.0;
    ...
    CHECK_THAT(dsys.A(0, 0), Catch::Matchers::WithinAbs(Ad, discretise_tolerance<double, 1>(std::abs(Ad))));
    CHECK_THAT(dsys.B(0, 0), Catch::Matchers::WithinAbs(Bd, discretise_tolerance<double, 1>(std::abs(Bd))));
    CHECK_THAT(dsys.C(0, 0), Catch::Matchers::WithinAbs(c, discretise_tolerance<double, 1>(std::abs(c))));
    CHECK_THAT(dsys.D(0, 0), Catch::Matchers::WithinAbs(d, discretise_tolerance<double, 1>(std::abs(d))));
}

TEST_CASE("backward euler discretise matches the closed-form biproper scalar system")
{
    ctrlpp::continuous_state_space<double, 1, 1, 1> sys{};
    const double s = -3.0;
    const double b = 2.0;
...
TEST_CASE("tustin prewarp discretise agrees with the plain bilinear map at the warped period")
{
    // Rescaling dt by dt_warp = (2/w_c) * tan(w_c*dt/2) before the bilinear map is the
    // definition of prewarping; discretise(tustin_prewarp{w_c}, sys, dt) must therefore equal
    // discretise(tustin{}, sys, dt_warp) exactly (both compute the same formula).
    ctrlpp::continuous_state_space<double, 1, 1, 1> sys{};
    sys.A(0, 0) = -4.0;
    sys.B(0, 0) = 1.0;
    sys.C(0, 0) = 1.0;
    ...
    auto dsys_warp = ctrlpp::discretise(ctrlpp::tustin_prewarp<double>{w_c}, sys, dt);

    const double dt_warp = (2.0 / w_c) * std::tan(w_c * dt / 2.0);
    auto dsys_ref = ctrlpp::discretise(ctrlpp::tustin{}, sys, dt_warp);

    CHECK_THAT(dsys_warp.A(0, 0), Catch::Matchers::WithinAbs(dsys_ref.A(0, 0), discretise_tolerance<double, 1>(std::abs(dsys_ref.A(0, 0)))));
    CHECK_THAT(dsys_warp.B(0, 0), Catch::Matchers::WithinAbs(dsys_ref.B(0, 0), discretise_tolerance<double, 1>(std::abs(dsys_ref.B(0, 0)))));
}
...
TEST_CASE("tustin prewarp maps the critical frequency onto the continuous imaginary axis exactly")
{
    ...
    const double tol = discretise_tolerance<double, 1>(w_c);

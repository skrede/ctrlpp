...
auto make_upper_bound_constraint(double bound) {
    return [bound](const Eigen::Vector2d& x,
                   const Eigen::Matrix<double, 1, 1>&)
        -> ctrlpp::Vector<double, NC> {
        return ctrlpp::Vector<double, NC>{x(0) - bound};
    };
}
...
auto make_terminal_constraint(double target) {
    return [target](const Eigen::Vector2d& x)
        -> ctrlpp::Vector<double, NTC_1> {
        return ctrlpp::Vector<double, NTC_1>{x(0) - target};
    };
}
...
struct diff_constraint {
    ...
    auto operator()(const Eigen::Vector2d& x,
                    const Eigen::Matrix<double, 1, 1>&) const
        -> ctrlpp::Vector<double, 1>
    {
        return ctrlpp::Vector<double, 1>{x(0) - bound};
    }

    auto jacobian(const Eigen::Vector2d&,
                  const Eigen::Matrix<double, 1, 1>&) const
        -> ctrlpp::Matrix<double, 1, NX + NU>
    {
...
struct diff_terminal_constraint {
    ...
    auto operator()(const Eigen::Vector2d& x) const
        -> ctrlpp::Vector<double, 1>
    {
        return ctrlpp::Vector<double, 1>{x(0) - target};
    }

    auto jacobian(const Eigen::Vector2d&) const
        -> ctrlpp::Matrix<double, 1, NX>
    {
...
TEST_CASE("constraint_model concept accepts valid path constraint lambda") {
    ...
    static_assert(ctrlpp::constraint_model<decltype(g), double, NX, NU, NC>,
        "path constraint lambda must satisfy constraint_model");
}

TEST_CASE("terminal_constraint_model concept accepts valid terminal constraint lambda") {
    auto h = make_terminal_constraint(0.5);
    static_assert(ctrlpp::terminal_constraint_model<decltype(h), double, NX, NTC_1>,
        "terminal constraint lambda must satisfy terminal_constraint_model");
}

TEST_CASE("differentiable_constraint concept accepts struct with jacobian") {
    static_assert(ctrlpp::differentiable_constraint<diff_constraint, double, NX, NU, NC>,
        "diff_constraint must satisfy differentiable_constraint");
}

TEST_CASE("differentiable_terminal_constraint concept accepts struct with jacobian") {
    static_assert(ctrlpp::differentiable_terminal_constraint<diff_terminal_constraint, double, NX, NTC_1>,
        "diff_terminal_constraint must satisfy differentiable_terminal_constraint");
}
...
TEST_CASE("nmpc with NC=0 NTC=0 produces same results as original", "[nmpc][constraint][compat]") {
    ...
    ctrlpp::nmpc<double, NX, NU, NloptSolver, decltype(double_integrator)>
        ctrl_old{double_integrator, config_old};
    ...
    ctrlpp::nmpc<double, NX, NU, NloptSolver, decltype(double_integrator), 0, 0>
        ctrl_new{double_integrator, config_new};
...
TEST_CASE("soft path constraint is approximately satisfied", "[nmpc][constraint][soft]") {
    ...
    ctrlpp::nmpc<double, NX, NU, NloptSolver, decltype(double_integrator), NC, 0>
        controller{double_integrator, config};
...
TEST_CASE("terminal constraint drives final state", "[nmpc][constraint][terminal]") {
    ...
    ctrlpp::nmpc<double, NX, NU, NloptSolver, decltype(double_integrator), 0, NTC_1>
        controller{double_integrator, config};
...
TEST_CASE("combined path and terminal constraints", "[nmpc][constraint][combined]") {
    ...
    ctrlpp::nmpc<double, NX, NU, NloptSolver, decltype(double_integrator), NC, NTC_1>
        controller{double_integrator, config};

...
{
    return rc::gen::map(rc::gen::container<std::vector<double>>(static_cast<std::size_t>(rows * cols), bounded_double(lo, hi)),
...
{
    return rc::gen::map(gen_random_matrix(nx, nx, -5.0, 5.0),
                        [](const Eigen::MatrixXd& Ld)
                        {
...
{
    return rc::gen::map(bounded_double(0.1, 10.0),
                        [](double v)
                        {
                            Mat1 m;
                            m << v;
...
{
    return rc::gen::suchThat(rc::gen::apply(
                             ...
                             [](const ControllableSystem& sys) { return ctrlpp::is_controllable<double, NX, NU>(sys.A, sys.B); });
}
...
TEST_CASE("dare riccati identity holds for random controllable systems", "[dare][property]")
{
    rc::prop("riccati residual is near zero for valid solutions",
             []
             {
                 ...
                 Mat2 residual = sys.A.transpose() * P * sys.A - P - sys.A.transpose() * P * sys.B * S_inv_BtPA + Q;

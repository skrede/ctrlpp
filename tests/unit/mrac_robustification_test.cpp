...
TEST_CASE("Sigma-modification adds leakage to parameters", "[mrac][sigma_modification]")
{
    ...
    REQUIRE(ctrl.evaluate(vec1(0.0), vec1(0.0)).has_value());
...
TEST_CASE("e-modification scales leakage by error magnitude", "[mrac][e_modification]")
{
    ...
    REQUIRE(ctrl_zero.evaluate(vec1(0.0), vec1(0.0)).has_value());
    ...
    REQUIRE(ctrl_large.evaluate(vec1(0.0), vec1(0.0)).has_value());

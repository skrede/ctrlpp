...
TEST_CASE("Dead-zone prevents parameter drift at steady state", "[mrac][dead_zone]")
{
    ...
    for(int k = 0; k < 10000; ++k)
    {
        ...
        if(k == 4999)
        {
            theta_x_5000 = ctrl.theta_x();
            theta_r_5000 = ctrl.theta_r();
        }
    }

    REQUIRE(std::isfinite(ctrl.theta_x()));
    REQUIRE(std::isfinite(ctrl.theta_r()));
    CHECK(std::abs(ctrl.theta_x() - theta_x_5000) < 1e-6);
    CHECK(std::abs(ctrl.theta_r() - theta_r_5000) < 1e-6);
}
...
TEST_CASE("Sigma-modification adds leakage to parameters", "[mrac][sigma_modification]")
{
    ...
    REQUIRE_THAT(ctrl.theta_x(), WithinAbs(0.95, 1e-10));
    REQUIRE_THAT(ctrl.theta_r(), WithinAbs(0.95, 1e-10));
}
...
TEST_CASE("Sigma-modification parameters remain bounded over long horizon", "[mrac][sigma_modification]")
{
    ...
    REQUIRE(std::isfinite(ctrl.theta_x()));
    REQUIRE(std::isfinite(ctrl.theta_r()));
    CHECK(std::abs(ctrl.theta_x()) < 1e6);
    CHECK(std::abs(ctrl.theta_r()) < 1e6);
}
...
TEST_CASE("e-modification scales leakage by error magnitude", "[mrac][e_modification]")
{
    ...
    auto theta_x_zero_error = ctrl_zero.theta_x();
    ...
    auto theta_x_large_error = ctrl_large.theta_x();

...
auto double_integrator_dynamics = [](const Eigen::Vector2d& x, const Eigen::Matrix<double, 1, 1>& u) -> Eigen::Vector2d { return Eigen::Vector2d{x(0) + dt * x(1) + 0.5 * dt * dt * u(0), x(1) + dt * u(0)}; };
...
TEST_CASE("linear mpc closed-loop convergence - double integrator", "[mpc][closedloop]")
{
...
TEST_CASE("nonlinear mpc closed-loop convergence - double integrator", "[nmpc][closedloop]")
{
...
TEST_CASE("linear mpc trajectory tracking with reference change", "[mpc][closedloop]")
{
    auto sys = make_double_integrator();

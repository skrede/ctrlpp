...
{
    static_assert(ctrlpp::dynamics_model<decltype(double_integrator), double, NX, NU>, "double_integrator must satisfy dynamics_model concept");
}

TEST_CASE("dynamics_model concept rejects lambda with wrong signature")
{
    auto wrong_sig = [](const Eigen::Vector2d& x) -> Eigen::Vector2d { return x; };
    static_assert(!ctrlpp::dynamics_model<decltype(wrong_sig), double, NX, NU>, "wrong signature must not satisfy dynamics_model concept");
}

TEST_CASE("mock_nlp_solver satisfies nlp_solver concept")
{
    static_assert(ctrlpp::nlp_solver<mock_nlp_solver>, "mock_nlp_solver must satisfy nlp_solver concept");
}
...
TEST_CASE("nmpc with mock solver", "[nmpc]")
{
    ...
    {
        Eigen::Vector2d x0{1.0, 0.0};
        std::vector<Eigen::Vector2d> refs(static_cast<std::size_t>(N + 1), Eigen::Vector2d{2.0, 0.0});
        auto result = controller.solve(x0, std::span<const Eigen::Vector2d>{refs});
        REQUIRE(result.has_value());
    }

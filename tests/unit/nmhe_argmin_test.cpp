...
using ArgminSolver = argmin_solver<double, argmin_slsqp>;
using NmheType = nmhe<double, NX, NU, NY, N, ArgminSolver, pendulum_dynamics, angle_measurement>;

}

TEST_CASE("nmhe argmin falls back to ekf during warmup", "[nmhe][argmin]")
{
...
TEST_CASE("nmhe argmin tracks nonlinear system after warmup", "[nmhe][argmin]")
{
    nmhe_config<double, NX, NU, NY, N> cfg;
    cfg.Q = Matrix<double, NX, NX>::Identity() * 0.01;
    cfg.R = Matrix<double, NY, NY>::Identity() * 0.1;
...
TEST_CASE("nmhe argmin covariance is finite", "[nmhe][argmin]")
{
    nmhe_config<double, NX, NU, NY, N> cfg;
    cfg.Q = Matrix<double, NX, NX>::Identity() * 0.01;
    cfg.R = Matrix<double, NY, NY>::Identity() * 0.1;
...
TEST_CASE("nmhe argmin satisfies CovarianceObserver", "[nmhe][argmin]")
{
    static_assert(ObserverPolicy<NmheType>);
    static_assert(CovarianceObserver<NmheType>);
}

TEST_CASE("nmhe argmin handles soft constraints", "[nmhe][argmin]")
{
    nmhe_config<double, NX, NU, NY, N> cfg;
    cfg.Q = Matrix<double, NX, NX>::Identity() * 0.01;
    cfg.R = Matrix<double, NY, NY>::Identity() * 0.1;

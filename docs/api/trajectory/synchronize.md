...
Synchronization rests on each axis reporting the duration it actually realizes rather than the one it was handed. No axis assigns itself the requested duration after its solve, so `duration()` afterwards is the sum of that axis's own realized phase durations. Where the retiming is not exact it is bounded, and the trapezoidal page states that bound in units in the last place along with the configurations that escape it -- both of them shapes whose cruise velocity has been driven to nothing, where the amplification belongs to the profile family rather than to the solve. An axis that cannot meet the target reports a typed failure instead of accepting it and realizing something else, which is what keeps a synchronized set synchronized.

...
Synchronization rests on each axis reporting the duration it actually realizes rather than the one it was handed. No axis assigns itself the requested duration after its solve, so `duration()` afterwards is the sum of that axis's own realized phase durations. Where the retiming is not exact it is bounded, and the trapezoidal page states that bound in units in the last place along with the one configuration that escapes it. An axis that cannot meet the target reports a typed failure instead of accepting it and realizing something else, which is what keeps a synchronized set synchronized.

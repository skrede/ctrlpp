...
| Riccati steady-state solve: `dare` / `care` | YES | YES (Eigen Schur iteration bound; sign-function Newton capped at `max_iters = 40` in `detail/care_sign_function.h`, followed by one fixed-size projector check and, only when that sufficient check is unresolved, fixed-size residual and closed-loop checks) | YES | YES | YES | `dare_care_nomalloc_test` (NX = 2, 4, 8 across the Schur, sign-function, and balanced-Schur variants); `care_convergence_anchor_test` (fixed-seed near-axis and simple-input scale sweeps); leg 1 witness calls `dare` and `care` |

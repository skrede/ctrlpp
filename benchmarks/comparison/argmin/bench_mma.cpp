// bench_mma.cpp
//
// Benchmarks MMA on the two reference plants (double integrator + pendulum)
// at NX in {2, 4, 8} and horizon N in {10, 20, 30}. Two problem suites per
// cell:
//
//   Suite A (auglag-wrapped multiple-shooting NMPC, equality continuity):
//     argmin side: NOT INSTANTIATED. The composition
//                  argmin_solver<double, argmin_auglag<argmin_mma>> fails
//                  to instantiate against the upstream argmin pin (probed
//                  via `g++ -std=c++23 -c` against the released milestone/
//                  v0.3.0 tag): mma_policy::init static-asserts
//                  `constrained<Problem>`, but auglag's inner subproblem
//                  satisfies only `differentiable + bound_constrained` (it
//                  is, by design, the unconstrained AL re-parameterisation
//                  for the inner solver). The two concepts are mutually
//                  exclusive in the current upstream API. Suite A argmin
//                  rows are therefore emitted as algorithm =
//                  "argmin_incompatible" with success=0; the upstream
//                  resolution would require argmin's auglag to expose the
//                  zero-constraint stub interface to inequality-only inner
//                  solvers (out of scope here).
//
//     NLopt side:  nlopt_solver<double> with nlopt_algorithm::auglag_mma
//                  (NLOPT_AUGLAG_EQ outer + NLOPT_LD_MMA inner via
//                  set_local_optimizer). Equality constraints are absorbed
//                  into the outer AUGLAG_EQ penalty; inequalities pass
//                  through to the inner LD_MMA. This path runs cleanly.
//
//   Suite B (single-shooting unconstrained NLP):
//     argmin side: argmin_solver<double, argmin_mma, true>. Note the
//                  `Constrained=true` flag is required even though the
//                  single-shooting problem has zero equality and zero
//                  inequality constraints: upstream mma_policy::init
//                  static-asserts `constrained<Problem>`, so the bridge
//                  type must be argmin_constrained_problem (which exposes
//                  a zero-row constraint accessor) rather than the simpler
//                  argmin_problem (which exposes none). Behaviourally
//                  equivalent on this NLP because n_constraints=0; the
//                  flag flip is a concept-satisfaction requirement only.
//     NLopt side:  nlopt_solver<double> with nlopt_algorithm::mma (raw
//                  LD_MMA on the box-constrained, equality-free NLP; no
//                  AUGLAG wrap needed).
//
// Warm-start configurations per cell: cold, primal_only, curvature.
// `curvature` exercises argmin's MMA asymptote-history carry; NLopt has no
// equivalent so the `curvature` NLopt rows are expected to equal the
// `primal_only` NLopt rows per cell. Documented feature, not a bench
// defect.
//
// NLopt max_eval semantics on AUGLAG: the outer opt counts TOTAL function
// evaluations including inner evaluations toward its own budget. To give
// the AUGLAG_EQ + LD_MMA composition room to converge instead of always
// terminating with max_iterations on the default 500-eval budget, this
// bench raises max_eval to 2000 and caps wall-clock per solve at 0.5 s on
// both backends. Both sides also receive matched ftol_rel / xtol_rel /
// constraint_tol so the comparison is not tainted by asymmetric
// termination thresholds. The wall-clock cap is the dominant termination
// criterion on the slow cells (NX=8 N=30 AUGLAG_EQ, and the upstream
// argmin MMA path on the box-only single-shooting problem where
// asymptote-update has no constraint signal and the solver iterates to
// the limit).
//
// Output: bench_mma_timing.csv (nanobench mustache) and
// bench_mma_quality.csv (bench_metrics.h schema).
...
using NablaRawMma = ctrlpp::argmin_solver<double, ctrlpp::argmin_mma, true>;
                 ...
                 ctrlpp::warm_start_mode ws_mode)
{
    ...
    // primal_only by construction.
    {
        ...
        write_quality_csv_row(quality_csv, system_name, "nlopt", "auglag_mma",
    ...
    // argmin auglag<mma> is unbuildable upstream; emit a sentinel row.
    write_argmin_incompatible_row(quality_csv, system_name, "auglag_mma",
                                  ws_label, static_cast<int>(NX), horizon);
}
                 ...
                 ctrlpp::warm_start_mode ws_mode)
{
    ...
    // ws_mode.
    {
        ...
        write_quality_csv_row(quality_csv, system_name, "nlopt", "mma",
    ...
    // values from argmin_mma_settings).
    {
        ...
        NablaRawMma solver{nabla_cfg};

        bench.run("argmin_mma",
                  [&]
                  {
                      solver.setup(problem);
                      auto r = solver.solve(update);
        ...
        auto r = solver.solve(update);
        auto qm = compute_quality_metrics(problem, r);
        write_quality_csv_row(quality_csv, system_name, "argmin", "mma",
                              ws_label, static_cast<int>(NX), horizon, qm);
    }
}
                     ...
                     std::ostream& quality_csv)
{
    ...
    nlopt_auglag_cfg.algorithm      = ctrlpp::nlopt_algorithm::auglag_mma;
    ...
    nlopt_raw_cfg.algorithm = ctrlpp::nlopt_algorithm::mma;
    ...
    write_rate("argmin", "auglag_mma", 0);
    write_rate("nlopt",  "auglag_mma", suite_a_nlopt_ok);
    write_rate("argmin", "mma",        suite_b_argmin_ok);
    write_rate("nlopt",  "mma",        suite_b_nlopt_ok);
}

#ifndef HPP_GUARD_CPPCTRL_EIGEN_PLACE_H
#define HPP_GUARD_CPPCTRL_EIGEN_PLACE_H
...
auto validate_conjugate_pairs(const std::array<std::complex<Scalar>, N>& poles) -> bool
{
    ...
    for (std::size_t i = 0; i < N; ++i) {
        ...
        for (std::size_t j = i + 1; j < N; ++j) {
            if (matched[j])
                continue;
            if (std::abs(poles[i].real() - poles[j].real()) < Scalar{1e-12} &&
                std::abs(poles[i].imag() + poles[j].imag()) < Scalar{1e-12}) {
                matched[i] = true;
                matched[j] = true;
                found = true;
                break;
...
template<typename Scalar, std::size_t N>
auto char_poly_coeffs(const std::array<std::complex<Scalar>, N>& poles)
    -> std::array<Scalar, N>
{
...
auto place(const Eigen::Matrix<Scalar, int(NX), int(NX)>& A,
...
auto place_observer(const Eigen::Matrix<Scalar, int(NX), int(NX)>& A,

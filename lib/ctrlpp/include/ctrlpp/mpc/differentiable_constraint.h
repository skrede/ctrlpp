...
concept differentiable_constraint = constraint_model<G, Scalar, NX, NU, NC> &&
    requires(const G& g,
...
concept differentiable_terminal_constraint = terminal_constraint_model<H, Scalar, NX, NTC> &&
    requires(const H& h,
             const Vector<Scalar, NX>& x) {

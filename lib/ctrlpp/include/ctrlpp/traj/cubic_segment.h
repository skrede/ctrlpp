#ifndef HPP_GUARD_CTRLPP_TRAJ_CUBIC_SEGMENT_H
#define HPP_GUARD_CTRLPP_TRAJ_CUBIC_SEGMENT_H
...
/// @brief Create cubic segment from boundary conditions q(0)=q0, q(T)=q1, dq(0)=v0, dq(T)=v1.
///
/// Coefficients derived from B&M eq. (2.2) in normalized time tau = t/T.
template <typename Scalar, int Rows>
auto make_cubic_segment(
    ...
    Scalar duration) -> cubic_segment<Scalar, static_cast<std::size_t>(Rows)>
{
...
static_assert(trajectory_segment<cubic_segment<double, 1>, double, 1>);

#ifndef HPP_GUARD_CTRLPP_SIGMA_POINTS_SO3_SIGMA_POINTS_H
#define HPP_GUARD_CTRLPP_SIGMA_POINTS_SO3_SIGMA_POINTS_H
...
concept manifold_sigma_point_strategy = requires {
    ...
    typename S::options_t;
              ...
              const Matrix<Scalar, 3, 3>& P) {
    { s.generate(q, P) }
        -> std::convertible_to<manifold_sigma_result<Scalar, S::num_points>>;
};
...
public:
    ...
    [[nodiscard]] auto generate(const Eigen::Quaternion<Scalar>& q_mean,
                                const Matrix<Scalar, 3, 3>& P) const
        -> manifold_sigma_result<Scalar, num_points>
    {
        auto tangent = inner_.generate(Vector<Scalar, tangent_dim>::Zero(), P);
...
private:
    merwe_sigma_points<Scalar, tangent_dim> inner_;
};

    ...
    -> nlp_problem<Scalar>
{
    ...
    std::function<Scalar(std::span<const Scalar>)> cost =
        [=](std::span<const Scalar> z) -> Scalar {
            Scalar total{0};
            ...
            total += Scalar{0.5} * dx0.dot(
                (arrival_weight * state->arrival_P_inv) * dx0);

            // Process noise cost: sum 0.5 * w_k^T Q_inv w_k
            for (int k = 0; k < Ni; ++k) {
                Eigen::Map<const Vector<Scalar, NX>> xk(
                    z.data() + x_offset + k * nx);
                Eigen::Map<const Vector<Scalar, NX>> xk1(
                    z.data() + x_offset + (k + 1) * nx);

                Vector<Scalar, NX> wk = xk1 - dynamics(
                    xk, state->u_window[static_cast<std::size_t>(k)]);
            ...
            for (int k = 0; k <= Ni; ++k) {
                ...
                Vector<Scalar, NY> vk =
                    state->z_window[static_cast<std::size_t>(k)] - measurement(xk);
    ...
    std::function<void(std::span<const Scalar>, std::span<Scalar>)> constraints =
        [=](std::span<const Scalar> z, std::span<Scalar> c) {
            ...
            for (int k = 0; k < Ni; ++k) {
                ...
                Vector<Scalar, NX> x_next = dynamics(
                    xk, state->u_window[static_cast<std::size_t>(k)]);

                for (int i = 0; i < nx; ++i) {
                    c[static_cast<std::size_t>(continuity_start + k * nx + i)] =
                        xk1[i] - x_next[i];
                }
            }
            ...
            if (has_residual) {
                ...
                for (int k = 0; k <= Ni; ++k) {
                    ...
                    Vector<Scalar, NY> residual =
                        state->z_window[static_cast<std::size_t>(k)]
                        - measurement(xk);

                    for (int i = 0; i < ny; ++i) {
                        std::size_t base = static_cast<std::size_t>(
                            residual_con_start + k * ny * 2 + i * 2);

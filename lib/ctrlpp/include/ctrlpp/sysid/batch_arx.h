...
[[nodiscard]] auto batch_arx(const Eigen::MatrixBase<Derived1>& Y,
    ...
    -> arx_result<typename Derived1::Scalar, NA, 1, 1>
{
    ...
    for (Eigen::Index i = 0; i < n_eff; ++i) {
        ...
        for (std::size_t j = 0; j < NB; ++j) {
            Phi(i, static_cast<Eigen::Index>(NA + j)) =
                U(0, row - static_cast<Eigen::Index>(j + 1));
        }
    ...
    Eigen::Matrix<Scalar, static_cast<int>(NA), 1> x =
        Eigen::Matrix<Scalar, static_cast<int>(NA), 1>::Zero();
